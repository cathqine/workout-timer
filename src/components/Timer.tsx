...
import { padding, secondsToMinutes } from "./helpers/helpers.tsx";
...
const Timer = (props: TimerProps): ReactElement => {
  ...
  const { mins, secs }: { mins: number, secs: number } = secondsToMinutes(Number(localStorage.getItem('seconds per set')));

  // let [minutes, setMinutes] = useState<number>(1);
  let [displayMins, setDisplayMins] = useState<number>(mins);

  // let [seconds, setSeconds] = useState<number>(3);
  let [displaySecs, setDisplaySecs] = useState<number>(secs);
